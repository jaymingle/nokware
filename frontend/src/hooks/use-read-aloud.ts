...
/**
 * Read a text aloud in parts: the second is fetched with the first, and each after that while the one before plays,
 * so a long answer starts in seconds. Play, pause and resume; stop another reading that is playing. A part that fails
 * can be carried on from. One audio element throughout, made on the first press, so later parts play without another.
 */
