...
Both answer with the audio itself (MP3). Only what the server produced is spoken, never anything about someone's
safety (read_aloud.py).
"""
...
def _audio(script: str) -> Response:
    spoken = read_aloud.audio(script, utc_now())
    return Response(spoken.data, media_type=spoken.content_type, headers={"Cache-Control": "no-store"})
...
def answer(request: SpeechAnswerRequest) -> Response:
    ...
    return _audio(read_aloud.answer_script(view.question, view.answer, view.status))
...
def report(request: SpeechReportRequest) -> Response:
    ...
    return _audio(read_aloud.status_script(public, receipt=request.kind == "receipt"))
