...
short label ([S1], [S2], ...) and the model cites those labels. Afterwards,
sanitize_citations() keeps only labels that map to a retrieved document and
...
# One citation group: [S1], [S1, S3], [S1; S2], [S1 and S2], (S2), [s 4], [Sources S1, S2] ...
_CITATION_GROUP = re.compile(
    r"[\[(]\s*(?:sources?\s*)?(S\s*\d+(?:\s*(?:,|;|&|and)\s*S\s*\d+)*)\s*[\])]",
    re.IGNORECASE,
)
_LABEL = re.compile(r"S\s*(\d+)", re.IGNORECASE)
...
def sanitize_citations(answer: str, valid_labels: set[str]) -> tuple[str, set[str]]:
    ...
    def keep_valid(match: re.Match[str]) -> str:
        labels = [f"{LABEL_PREFIX}{number}" for number in _LABEL.findall(match.group(1))]
