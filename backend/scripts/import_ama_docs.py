...
imported once; the extra copy is reported and skipped.
    ...
    backend/.venv/bin/python backend/scripts/import_ama_docs.py [--docs-dir DIR] [--limit N] [--only TEXT] [--dry-run]
...
def parse_year(raw: str) -> int | None:
    """The manifest year is scraped from titles; keep it only if it is a plausible past year."""
    year = int(raw) if str(raw).isdigit() else None
...
def to_entry(raw: dict[str, Any], docs_dir: Path, categories: dict[int, str]) -> Entry:
    ...
    return Entry(
        ...
        team=team,
        document_year=parse_year(raw["year"]),
...
def import_entry(entry: Entry, uploaded_by: str, docs_dir: Path, dry_run: bool) -> tuple[str, str]:
    """Run every step not already done. Returns (outcome, detail)."""
    steps = []
    if not ledger_file_exists(entry.object_name):
        if not dry_run:
            upload_ledger_file((docs_dir / entry.file).read_bytes(), Path(entry.file).name, entry.object_name)
        steps.append("would upload" if dry_run else "uploaded")
    document = find_document(entry.document_id)
    if document is None:
        if not dry_run:
            create_document(entry.document_id, entry.record(uploaded_by))
        steps.append("would create record" if dry_run else "record created")
    elif document.get("ingestedAt"):
        return ("done", "already imported") if not steps else ("imported", " · ".join(steps))
    if dry_run:
...
def run(entries: list[Entry], uploaders: dict[str, str], docs_dir: Path, dry_run: bool) -> dict[str, list[str]]:
    outcomes: dict[str, list[str]] = {"imported": [], "done": [], "not_searchable": [], "failed": []}
    for number, entry in enumerate(entries, 1):
        label = f"[{number}/{len(entries)}] {entry.file[:62]:62}"
        try:
            outcome, detail = import_entry(entry, uploaders[entry.team], docs_dir, dry_run)
...
def print_summary(outcomes: dict[str, list[str]], duplicates: list[tuple[str, str]]) -> None:
    print(
        f"\nImported {len(outcomes['imported'])}, already done {len(outcomes['done'])}, "
...
def main() -> int:
    ...
    try:
        outcomes = run(entries, uploaders, args.docs_dir, args.dry_run)
